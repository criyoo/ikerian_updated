...
resource "aws_lambda_function" "data_processor" {
  ...
  environment {
    variables = {
      RAW_DATA_BUCKET       = var.raw_data_bucket.bucket
      PROCESSED_DATA_BUCKET = var.processed_data_bucket.bucket
    }
  }
...
resource "aws_lambda_permission" "s3_trigger" {
  ...
  source_arn    = var.raw_data_bucket.arn

  depends_on = [aws_lambda_function.data_processor, var.raw_data_bucket, var.processed_data_bucket]
}
