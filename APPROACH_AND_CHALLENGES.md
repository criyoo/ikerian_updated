...
- **Security Level**: Standard AWS security practices is sufficient, operational overhead of key management is seeded to AWS due to team size, IAM permission is sufficient to define who or what services should have access, cost implication need to be kept low and encryption across other AWS services will use different keys for seperation of concerns.
...
- **Data Access Pattern**: The access pattern of data is not known, therefore s3 intelligent tiering is incorporated to move data between access tier to minimise storage cost
...
**Solution**:  I missed configured the s3 bucket notification, I needed to ensure S3 event notifications was correctly wired to Lambda using the `aws_s3_bucket_notification` resource and the correct permission granted.
...
**Solution**: I reviewed the resource naming and used consistent naming conventions across the project and also used environment prefixes
