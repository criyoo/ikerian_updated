...
def validate_record(record: Dict[str, Any], record_index: int) -> Dict[str, Any]:
    """
    Function to validate raw data fields used in generating the processed data.
    """
    validation_errors = []
    ...
    if not isinstance(record, dict):
        raise ValueError(
            f"Record {record_index}: Must be a dictionary, got {type(record).__name__}"
        )

    # Validate patient_id field
    if "patient_id" not in record:
        validation_errors.append(
            f"Record {record_index}: Missing required field 'patient_id'"
        )
    elif not isinstance(record["patient_id"], str):
        validation_errors.append(
            f"Record {record_index}: 'patient_id' must be a string, got {type(record['patient_id']).__name__}"
        )
    elif not record["patient_id"].strip():
        validation_errors.append(
            f"Record {record_index}: 'patient_id' cannot be empty or whitespace"
        )

    # Validate patient_name field
    if "patient_name" not in record:
        validation_errors.append(
            f"Record {record_index}: Missing required field 'patient_name'"
        )
    elif not isinstance(record["patient_name"], str):
        validation_errors.append(
            f"Record {record_index}: 'patient_name' must be a string, got {type(record['patient_name']).__name__}"
        )
    elif not record["patient_name"].strip():
        validation_errors.append(
            f"Record {record_index}: 'patient_name' cannot be empty or whitespace"
        )
...
def validate_data_structure(data: Any) -> List[Dict[str, Any]]:
    """
    Function to validate the overall raw data structure and return validated records.
    """
    ...
    for i, record in enumerate(data):
        try:
            validated_records.append(validate_record(record, i))
...
def lambda_handler(event, context):
    ...
    try:
        ...
        )  # Read JSON data from raw bucket, content is a list of dictionaries
        ...
        try:
            ...
            logger.info(
                f"Data validation successful: {len(validated_data)} records validated"
            )
        ...
        logger.info(
            f"Successfully wrote processed data to {processed_bucket}/{output_key}"
        )
